...
export default function EditProjectPage({
  ...
  useEffect(() => {
    if (project) {
      ...
      if (project.problemSolution) {
        setProblemSolution({
          problem: project.problemSolution.problem || "",
          solution: project.problemSolution.solution || "",
        });
      }
  ...
  useEffect(() => {
    const onPaste = (e: ClipboardEvent) => {
      ...
      if (file) {
        // If they click on the cover area or similar, but for simplicity,
        // we'll just allow setting cover or adding extra.
        // In edit mode, maybe we don't automatically override the cover on first paste
        // unless they are focused on it.
        // For now, let's follow the create logic.
  ...
  }, [image, imagePreview, setCoverFile, addExtraFiles]);
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] text-center">
        ...
        <button
          onClick={() => router.back()}
          className="px-6 py-2 border primary-border rounded-xl hover:text-[#0abab5] transition-colors"
        >
          Go Back
        </button>
      </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 p-2">
                    ...
                    {existingImages.map((src, i) => (
                      ...
                      >
                        ...
                        />
                        <div className="absolute top-1 left-1 px-1.5 py-0.5 rounded-md bg-black/50 text-[8px] font-bold text-white/50 uppercase tracking-widest">
                          Server
                        </div>
                        <button
                          type="button"
                          onClick={() => removeExistingImage(i)}
                          className="absolute top-1 right-1 p-1 rounded-lg bg-black/70 text-red-400 opacity-0 group-hover:opacity-100 transition-opacity shadow-lg"
                        >
                          <Trash2 size={12} />
                        </button>
                      </div>
                    ...
                    {imagePreviews.map((src, i) => (
                      <div
                        key={`new-${i}`}
                        className="relative aspect-video rounded-xl overflow-hidden group border border-[#0abab5]/30"
                      >
                        <Image
                          src={src}
                          alt={`new-${i}`}
                        ...
                        />
                        <div className="absolute top-1 left-1 px-1.5 py-0.5 rounded-md bg-[#0abab5]/50 text-[8px] font-bold text-white uppercase tracking-widest">
                          New
                        </div>
                        <button
                          type="button"
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className={cardCls}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="space-y-2 group">
                  <label className={labelCls}>
                    <LinkIcon size={12} /> Frontend GitHub
                  </label>
                  <input
                    name="fgithubUrl"
                ...
                <div className="space-y-2 group">
                  <label className={labelCls}>
                    <LinkIcon size={12} /> Backend GitHub
                  </label>
                  <input
                    name="bgithubUrl"
