        ...
        <div className="min-h-screen bg-transparent text-white flex flex-col lg:flex-row font-outfit relative">
            ...
            >
                <div className="p-6 flex items-center justify-between">
                    {/* Plain fade instead of framer-motion */}
                    <div
                        className={`transition-all duration-200 overflow-hidden ${isSidebarOpen ? "opacity-100 max-w-xs" : "opacity-0 max-w-0"
                            }`}
                    >
                        <h1 className="text-xl font-bold primary-text whitespace-nowrap">Admin</h1>
                    </div>
                    <button
                        ...
                        >
                            <link.icon
                                ...
                                className={
                                    pathname === link.href
                                        ? "text-[#0abab5]"
                                        : "primary-text4 group-hover:text-white"
                                }
                            />
                            {isSidebarOpen && (
                                <span className="font-medium whitespace-nowrap">{link.name}</span>
                            )}
                            {/* Plain dot instead of motion.div layoutId */}
                            {pathname === link.href && isSidebarOpen && (
                                <div className="ml-auto w-1.5 h-1.5 primary-rounded primary-color2" />
                            )}
                        </Link>
                    ))}
                </nav>
                ...
                <div className="flex items-center justify-around py-3 px-2 max-w-md mx-auto">
                    {visibleLinks.map((link) => (
                        <Link
                            ...
                            className={`flex flex-col items-center gap-1.5 w-full transition-colors ${pathname === link.href ? "text-[#0abab5]" : "text-white/40 hover:text-white"
                                }`}
                        >
                            <div
                    ...
                    <button
                        ...
                        className={`flex flex-col items-center gap-1.5 w-full transition-colors ${isMoreMenuOpen ? "text-[#0abab5]" : "text-white/40"
                            }`}
                    >
                        <div
                            className={`relative flex items-center justify-center p-2 primary-rounded transition-all ${isMoreMenuOpen ? "bg-white/10 ring-1 ring-white/20" : "hover:bg-white/5"
                                }`}
                        >
            ...
            {/* More Menu Overlay — plain CSS transition instead of framer-motion */}
            {isMoreMenuOpen && (
                <>
                    <div
                        onClick={() => setIsMoreMenuOpen(false)}
                    ...
                    />
                    <div className="fixed bottom-[calc(4rem+8px)] left-4 right-4 z-60 bg-zinc-900/95 backdrop-blur-3xl border border-white/10 primary-rounded overflow-hidden shadow-2xl lg:hidden">
