...
export default function EditCertificatePage() {
  ...
  const { id } = useParams();
  const { certificate, isLoading, isError, refetch } = useGetCertificateById(
    id as string,
  );
  ...
  useEffect(() => {
    if (certificate) {
      setFormData({
        ...
        timeTaken: certificate.timeTaken || "",
      ...
      setChallenges(
        certificate.challenges?.length ? certificate.challenges : [""],
      );
      setSkillsLearned(
        certificate.skillsLearned?.length ? certificate.skillsLearned : [""],
      );
      if (certificate.image?.url) {
        setImagePreview(certificate.image.url);
      }
    }
  }, [certificate]);
  ...
  const handleDelete = () => {
    if (
      ...
      deleteCertificate(id as string, {
        onSuccess: () => {
  ...
  if (isError) {
    return (
      <div className="text-center py-40 space-y-6">
        ...
        <button
          onClick={() => refetch()}
          className="px-6 py-2 bg-[#0abab5] text-black rounded-lg"
        >
          Retry
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mb-10">
        ...
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-black uppercase tracking-[0.2em] text-white">
            Edit <span className="text-[#0abab5]">Certificate</span>
          </h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            >
              ...
              />
              <span>{isDeleting ? "Deleting..." : "Delete Certificate"}</span>
            </button>
          </div>
        ...
        <div className="flex items-center justify-end gap-6 pt-10 border-t primary-border">
          ...
          >
            ...
            <span>{isUpdating ? "Updating..." : "Update Certificate"}</span>
          </button>
        </div>
      </form>
    </div>
