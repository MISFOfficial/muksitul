          ...
          <ul className="flex gap-2 items-center justify-center">
            {navLinks.map((link) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-bold transition-all primary-rounded hover:scale-105 cursor-pointer ${
                  link.name === "Resume"
            ...
            <div className="px-6 py-4 space-y-2 text-center">
              {navLinks.map((link) => (
                <button
                  key={link.name}
                  onClick={() => handleNavClick(link.id, link.href)}
                  className={`w-full block px-4 py-3 text-sm font-bold transition-all primary-rounded hover:scale-105 cursor-pointer ${
                    link.name === "Resume"
                      ? "primary-color2 text-black shadow-lg"
                      : "text-white/60 hover:text-white hover:bg-white/5 border primary-border"
                  }`}
                >
                  {link.name}
                </button>
